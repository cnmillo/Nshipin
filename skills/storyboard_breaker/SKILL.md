...
**视觉风格适配**：视频提示词必须与 `drama.style` 保持一致。系统会自动在视频生成时注入风格关键词（如 anime/ghibli/cinematic 等），Agent 只需确保 `video_prompt` 中的动作描述和场景描述准确即可，无需手动添加风格限定词。
...
- 多角色对话必须按角色分行，每行格式为 `角色名：台词内容`，例如：
  ```
  妈妈：今天怎么啦，晓晨？
  晓晨：没什么，妈妈。
  ```
  **禁止**将多个角色的台词写在同一行，如 `妈妈：今天怎么啦？晓晨：没什么。` 是错误格式
